...
var (
	...
	operations          = flag.Uint64("operations", ^uint64(0), "Number of operations to perform. If set non zero then test_duration must not be set")
	testDuration        = flag.Duration("test_duration", 0*time.Second, "If set to non-zero runs test for specified duration and operations flag must not be set")
