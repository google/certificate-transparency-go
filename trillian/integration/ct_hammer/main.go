...
type generatorFactory func(c *configpb.LogConfig) (integration.ChainGenerator, error)
...
func copierGeneratorFactory(ctx context.Context) generatorFactory {
...
// that create synthetic certificates (details of which are specified by the command line arguments).
func syntheticGeneratorFactory() generatorFactory {
...
func main() {
	...
	if len(*srcLogURI) > 0 || len(*srcLogName) > 0 {
		...
		generatorFactory = syntheticGeneratorFactory()
	}
