...
// STHProvider provides STHs of a source log to be served from a mirror.
// TODO(pavelkalinnikov): Add a default implementation.
type STHProvider interface {
	GetLimitedSTH(ctx context.Context, maxTreeSize int64) (*ct.SignedTreeHead, error)
}
...
func (sg *MirrorSTHGetter) GetSTH(ctx context.Context) (*ct.SignedTreeHead, error) {
	...
	sth, err := sg.sp.GetLimited(ctx, slr.TreeSize)
	if err != nil {
		return nil, err
	}
...
func getSignedLogRoot(ctx context.Context, client trillian.TrillianLogClient, logID int64, prefix string) (*trillian.SignedLogRoot, error) {
	...
	if q := ctx.Value(&quotaKey); q != nil {
		quotaUser, ok := q.(string)
		if !ok {
			return nil, fmt.Errorf("incorrect quota value: %v, type %T", q, q)
		}
		req.ChargeTo = appendUserCharge(req.ChargeTo, q.(string))
	}
