...
// ValidateLogConfig checks that a signle log config is valid.
func ValidateLogConfig(cfg *configpb.LogConfig) error {
	switch {
	case cfg.LogId == 0:
		return errors.New("empty log ID")
	case cfg.PrivateKey == nil && !cfg.IsMirror:
...
// 7. All mirror logs have public keys.
// 8. All public keys are valid.
// 9. All non-mirror logs have private keys.
func ValidateLogMultiConfig(cfg *configpb.LogMultiConfig) (map[string]*configpb.LogBackend, error) {
	...
	for _, backend := range cfg.Backends.Backend {
		...
		if len(backend.BackendSpec) == 0 {
			return nil, fmt.Errorf("empty backend spec for backend: %v", backend)
		}
